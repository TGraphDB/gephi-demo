TGraph Demo
----------------
> A demonstration of [TGraphDB](http://dx.doi.org/10.1145/2983323.2983335) build on top of Gephi (as a plugin).
>
> TGraphDB is a database designed for temporal graph data.

This Gephi plugin shows the power of TGraphDB. 
It demonstrates an (OLAP) road transportation network traffic system of Beijing city, with about 120K roads
and traffic data from 2010-11-04 00:00 to 2010-11-08 23:55 (about 90M points per day).

[//]: # ()
[//]: # (This plugin also provide a Database-Builder to help you build a fresh TGraph database from raw data files.)

Watch [this video](https://youtu.be/nbWa_5OL3GU) ([this](http://mashuai.buaa.edu.cn/TGraphdemo.avi) 
or [this](http://amitabha.water-crystal.org/TGraphDemo.mp4) if the first broken) 
to know what this plugin demonstrate in 3 minutes.

## System requirement
- Java Runtime Environment version 11 (or higher, not tested)
- Gephi 0.10.0
...
## 2025-07
* Packed with Gephi 0.10.0 with TGraph 4.4 which is compatible with Neo4j 4.4.37.
* add debug and enlarge memory usage in `%gephi-plugin-root%\target\gephi-0.10.0\etc\gephi.conf`

```
-J-Xmx12g -J-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=*:5005 
```
